...
namespace Maker.Hevadea.Game.Tiles
{
    public class Tile
    {
        ...
        public Tile(byte id)
        {
            ...
            TILES.ById[ID] = this;
            Sprite = new Sprite(Ressources.TileTiles, 0);
            DirtSprite = new Sprite(Ressources.TileTiles, 0);
        }
        ...
        public virtual void Draw(SpriteBatch spriteBatch, GameTime gameTime, Level level, TilePosition pos)
        {
            ...
            var u = level.GetTile(pos.X, pos.Y - 1) == this;
            var d = level.GetTile(pos.X, pos.Y + 1) == this;
            var l = level.GetTile(pos.X - 1, pos.Y) == this;
            var r = level.GetTile(pos.X + 1, pos.Y) == this;

            var ul = level.GetTile(pos.X - 1, pos.Y - 1) == this;
            var ur = level.GetTile(pos.X + 1, pos.Y - 1) == this;
            var dl = level.GetTile(pos.X - 1, pos.Y + 1) == this;
            var dr = level.GetTile(pos.X + 1, pos.Y + 1) == this;
