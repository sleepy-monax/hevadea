...
namespace Hevadea.Scenes
{
    public static class GuiHelper
    {
        ...
        public static void DrawBox(this SpriteBatch sb, Rectangle Bound, int size)
        {
            ...
            Bottom.Draw(sb, new Rectangle(Bound.X + size, Bound.Y + Bound.Height - size, Bound.Width - size * 2, size), Color.White);

            Left.Draw(sb, new Rectangle(Bound.X, Bound.Y + size, size, Bound.Height - size * 2), Color.White);
            Right.Draw(sb, new Rectangle(Bound.X + Bound.Width - size, Bound.Y + size, size, Bound.Height - size * 2), Color.White);
        }
    }
}
