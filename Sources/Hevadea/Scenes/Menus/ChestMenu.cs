...
namespace Hevadea.Scenes.Menus
{
    public class ChestMenu : Menu
    {
        ...
        public ChestMenu(Entity entity, Entity chest, GameManager game) : base(game)
        {
            ...
            Content = GuiHelper.CreateSplitContainer(new Rectangle(0, 0, 64, 64), "Inventory", _inventoryA, "Chest", new DockContainer()
            {
                Childrens = {                             new Button
                                { Font  = Ressources.FontRomulus, Text = "Back", Padding = new Padding(4) }
                                .RegisterMouseClickEvent((sender) => {Game.CurrentMenu = new HudMenu(Game);}),_inventoryB },
                Dock = Dock.Fill
            });
        }
