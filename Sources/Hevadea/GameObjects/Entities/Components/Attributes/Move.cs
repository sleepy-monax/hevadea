...
namespace Hevadea.GameObjects.Entities.Components.Attributes
{
    public class Move : Component
    {
        ...
        {
            var dir = new Vector2(x - Entity.X, y - Entity.Y);
        ...
        public bool Do(float sx, float sy, Direction facing)
        {
            ...
            if (Entity.Level.GetTile(Entity.GetTilePosition()).HasTag<Tags.Ground>())
            {
                var ground = Entity.Level.GetTile(Entity.GetTilePosition()).Tag<Tags.Ground>();
            ...
            if (Entity.X + sx >= Entity.Level.Width * ConstVal.TileSize) sx = 0;
            if (Entity.Y + sy >= Entity.Level.Height * ConstVal.TileSize) sy = 0;
            if (Entity.X + sx < 0) sx = 0;
            if (Entity.Y + sy < 0) sy = 0;

            var level = Entity.Level;
            var ownerColider = Entity.GetComponent<Colider>();
            ...
            if (ownerColider != null)
            {
                ...
                foreach (var e in colidingEntity)
                {
                    ...
                    if (e == Entity || !(eColider?.CanCollideWith(e) ?? false)) continue;

                    var eHitbox = eColider.GetHitBox();

                    if (Colision.Check(eHitbox.X, eHitbox.Y, eHitbox.Width, eHitbox.Height, ownerhitbox.X, ownerhitbox.Y + sy, ownerhitbox.Width, ownerhitbox.Height)) { e.GetComponent<Pushable>()?.Push(Entity, 0f, sy); IsMoving = true; }
                    ...
                    if (Colision.Check(eHitbox.X, eHitbox.Y, eHitbox.Width, eHitbox.Height, ownerhitbox.X + sx, ownerhitbox.Y, ownerhitbox.Width, ownerhitbox.Height)) { e.GetComponent<Pushable>()?.Push(Entity, sx, 0f); IsMoving = true;}
                    eHitbox = eColider.GetHitBox();
                ...
                var entityTilePosition = Entity.GetTilePosition();
                ...
                for (var x = -1; x <= 1; x++)
                {
                    for (var y = -1; y <= 1; y++)
                    {
                        ...
                        var isPassableTile = level.GetTile(tile).Tag<Tags.Solide>()?.CanPassThrought(Entity) ?? true;
            ...
            if (sx != 0 || sy != 0)
            {
                Entity.SetPosition(Entity.X + sx, Entity.Y + sy);
                Entity.Level.GetTile(Entity.GetTilePosition()).Tag<Tags.Ground>()?.SteppedOn(Entity, Entity.GetTilePosition());
