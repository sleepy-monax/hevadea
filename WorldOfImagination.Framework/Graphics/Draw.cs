...
namespace WorldOfImagination.Framework.Graphics
{
    public class Draw
    {
        ...
        public void Texture(Texture texture, Rectangle sourceRectangle, Rectangle destinationRectangle, Vector3 rotation)
        {
            texture.Bind(0);

            shader.SetUniformVariable("transform",      new Transform(new Vector3(destinationRectangle.X / 100, destinationRectangle.Y / 100, -10), rotation, new Vector3(destinationRectangle.Width / 100, destinationRectangle.Height / 100, 1)).GetMatrix());
