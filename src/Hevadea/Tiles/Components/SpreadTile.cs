...
namespace Hevadea.Tiles.Components
{
    public class Spread : TileComponent, IUpdatableTileComponent
    {
        ...
        public void Update(Tile tile, Coordinates coords, Dictionary<string, object> data, Level level, GameTime gameTime)
        {
            if (Rise.Rnd.Next(SpreadChance) == 0)
            {
                ...
                if (SpreadTo.Contains(level.GetTile(coords.X + p.X, coords.Y + p.Y))) level.SetTile(coords.X + p.X, coords.Y + p.Y, AttachedTile);
            }
        }
    }
}