...
namespace Hevadea.Framework.Utils
{
    public class PerlinNoise
    {
        ...
        public double Perlin(double x, double y, double z)
        {
            ...
            int a = _permutation[xi] + yi; // This here is Perlin's hash function.  We take our x value (remember,
            int aa = _permutation[a] + zi; // between 0 and 255) and get a random value (from our p[] array above) between
            int ab = _permutation[a + 1] + zi; // 0 and 255.  We then add y to it and plug that into p[], and add z to that.
            int b = _permutation[xi + 1] + yi; // Then, we get another random value by adding 1 to that and putting it into p[]
            int ba = _permutation[b] + zi; // and add z to it.  We do the whole thing over again starting with x+1.  Later
            int
                bb = _permutation[b + 1] + zi; // we plug aa, ab, ba, and bb back into p[] along with their +1's to get another set.
