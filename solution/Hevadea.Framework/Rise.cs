...
namespace Hevadea.Framework
{
    public static class Rise
    {
        ...
        private static void MonoGameOnUpdate(Game sender, GameTime gameTime)
        {
            ...
            if (Input.KeyTyped(Keys.F1)) Debug.HELP = !Debug.HELP;

            if (Input.KeyTyped(Keys.F2)) Debug.GAME = !Debug.GAME;

            if (Input.KeyTyped(Keys.F3)) Debug.GENERAL = !Debug.GENERAL;

            if (Input.KeyTyped(Keys.F4)) Debug.UI = !Debug.UI;

            if (Input.KeyTyped(Keys.F5)) Ui.Enabled = !Ui.Enabled;
